...
const DICT = new Map([
    ...
    ['رو', 'را'], ['می شود', 'می‌شود'], ['نمی شود', 'نمی‌شود'],
...
function spellCheck(text) {
    ...
    // ─── گام ۰: نرمال‌سازی اولیه (نیم‌فاصله‌های موجود را حفظ کن) ───
    // ZWNJ (U+200C) را با یک کاراکتر امن جایگزین موقت کن
    const ZWNJ = '\u200C';
    const ZWNJ_PLACEHOLDER = '\uFFFC';
    result = result.replace(new RegExp(ZWNJ, 'g'), ZWNJ_PLACEHOLDER);

    // ─── گام ۱: عبارات چندکلمه‌ای (با فاصلهٔ کامل) ───
    const multiWord = [...DICT.entries()].filter(([k]) => k.includes(' ')).sort((a, b) => b[0].length - a[0].length);
    ...
    // ─── گام ۲: کلمات تکی (بدون فاصله) ───
    const singleWord = [...DICT.entries()].filter(([k]) => !k.includes(' ')).sort((a, b) => b[0].length - a[0].length);
    ...
    // ─── گام ۳: اصلاح نیم‌فاصله برای «می» و «نمی» (فقط اگر فاصلهٔ کامل باشد) ───
    ...
    // ─── گام ۵: نیم‌فاصله برای «ها»ی جمع (فقط اگر فاصلهٔ کامل باشد) ───
    ...
    result = result.replace(new RegExp(ZWNJ_PLACEHOLDER, 'g'), ZWNJ);
...
function deepAnalyze(text) {
    ...
    return { analysis: basic, frequency: sf, unique_word_count: uq.size, text_type: tt, lexical_richness: { ttr: Math.round(ttr * 100) / 100 }, sentiment: { label: sent, positive_words: pc, negative_words: nc }, readability: { score: Math.round(re), level: re > 70 ? 'ساده' : re > 50 ? 'متوسط' : 'دشوار' }, reading_time_minutes: Math.ceil(words.length / 200), metadata: { version: '8.0.0', page: '218', processed_at: new Date().toISOString() } };
}

// ═══════════════════════════════════════════════════════════
app.get('/api', (req, res) => res.json({ name: 'ناتیق ۸.۰', version: '8.0.0', page: '218', status: 'فعال', patterns: DICT.size, whitelist: WHITELIST.size, algorithm: 'Multi-Pass Professional' }));
app.get('/api/health', (req, res) => res.json({ status: 'فعال', version: '8.0.0', patterns: DICT.size, whitelist: WHITELIST.size, timestamp: new Date().toISOString() }));

app.post('/api/spell', (req, res) => {
    try {
        const { text } = req.body;
        if (!text || !text.trim()) return res.status(400).json({ error: 'متن الزامی' });
        res.json({ success: true, ...spellCheck(text), version: '8.0.0' });
...
app.post('/api/chat', (req, res) => {
    try {
        ...
        if (lm === 'سلام' || lm === 'hi') return res.json({ reply: '🌟 ناتیق ۸.۰ — الگوریتم حرفه‌ای و بدون نقص\n' + DICT.size + ' الگو | ' + WHITELIST.size + ' کلمهٔ صحیح\n«ویرایش: متن» را تست کنید.' });
        if (lm === 'راهنما') return res.json({ reply: '📘 «ویرایش: متن» ← اصلاح کامل' });
        if (lm === 'سلامت') return res.json({ reply: '✅ ناتیق ۸.۰ | ' + DICT.size + ' الگو | ' + WHITELIST.size + ' کلمه | بدون نقص' });
...
app.listen(PORT, () => console.log('🚀 ناتیق ۸.۰ — Professional | ' + DICT.size + ' الگو | پورت ' + PORT));
module.exports = app;
